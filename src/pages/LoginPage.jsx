...
export default function LoginPage({ onGoRegister }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend returns { token }. Decode payload to get user info.
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <input type="email" placeholder="Email Address" value={email} onChange={e => setEmail(e.target.value)}
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 transition" required />
            ...
            <p className="text-center text-gray-400 text-sm">
              ...
              <button type="button" onClick={onGoRegister} className="text-purple-400 hover:text-purple-300">Register</button>
            </p>
          </form>
        </div>
      </div>
